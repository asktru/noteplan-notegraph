...
async function ensureSharedResources() {
  ...
  if (!match) {
    await CommandBar.prompt('Shared Resources needed',
      'Note Graph needs the "Shared Resources" (np.Shared) plugin. Please install it from NotePlan\'s plugin list.');
    return;
  }
...
async function onUpdateOrInstall() {
  ...
  catch (e) { console.log('Note Graph onUpdateOrInstall failed: ' + (e && e.message ? e.message : String(e))); }
}
